...
def ensure_rust_session_id_columns(apps, schema_editor):
    ...
    if "rust_session_id" not in ps_columns:
        if "user_id" in ps_columns:
            rename_column(ps_table, "user_id", "rust_session_id")
        else:
            field = models.CharField(max_length=255, null=True)
            field.set_attributes_from_name("rust_session_id")
            schema_editor.add_field(PlayerSession, field)

    lb_columns = column_names(lb_table)
    if "rust_session_id" not in lb_columns:
        if "user_id" in lb_columns:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("office", "0005_email_verification_and_email_unique"),
    ]

    operations = [
        migrations.SeparateDatabaseAndState(
            database_operations=[
                migrations.RunPython(ensure_rust_session_id_columns, migrations.RunPython.noop),
            ],
            state_operations=[
                migrations.AddField(
